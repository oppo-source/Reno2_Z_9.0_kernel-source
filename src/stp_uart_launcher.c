...
#define  WCN_XO_WCN_CLK_FAIL_COUNT          "persist.vendor.connsys.xowcn.fail.count"
...
static int cmd_hdr_update_patch_version(struct stp_params_config *config);
static int cmd_hdr_xowcn_clk_fail_count_log(struct stp_params_config *configs);
...
struct cmd_hdr g_cmd_hdr_table[] = {
    ...
    { "update_patch_version", cmd_hdr_update_patch_version},
    { "xo_wcn_clk_fail_count", cmd_hdr_xowcn_clk_fail_count_log},
};
...
static void launcher_set_patch_version()
{
    int ret = 0, update_bt = 1;
    char property[PROPERTY_VALUE_MAX];
    const char patch_prop[] = WCN_COMBO_PATCH_BUILD_VER_PROP;
    const char bt_prop[] = WCN_BT_BUILD_VER_PROP;
    struct wmt_vendor_patch patch;

    patch.id = cp_get_patch_id(g_wmt_fd, "patch");
    ioctl(g_wmt_fd, WMT_IOCTL_GET_ACTIVE_PATCH_VERSION, &patch);
    /* if fail to get active version, use vendor version instead */
    if (strlen(patch.version) < CP_VERSION_LENGTH) {
        cp_get_vendor_patch(g_wmt_fd, &patch);
        update_bt = 0;
    }

    ret = property_set(patch_prop, patch.version);
    if (0 != ret)
        ALOGE("set property(%s) to %s failed,ret:%d, errno:%d\n",
               patch_prop, patch.version, ret, errno);
    else
        ALOGI("set property(%s) to %s succeed.\n",
               patch_prop, patch.version);

    if (update_bt) {
        patch.id = cp_get_patch_id(g_wmt_fd, "ram_bt");
        if (patch.id == -1)
            patch.id = cp_get_patch_id(g_wmt_fd, "patch");
        ioctl(g_wmt_fd, WMT_IOCTL_GET_ACTIVE_PATCH_VERSION, &patch);
        ret = property_get(bt_prop, property, "-1");
        if (ret > PATCH_BUILD_VER_LEN) {
            snprintf(property + ret - strlen(patch.version), sizeof(property), "%s", patch.version);
            ret = property_set(bt_prop, property);
            if (0 != ret)
                ALOGE("set property(%s) to %s failed,ret:%d, errno:%d\n",
                       bt_prop, property, ret, errno);
            else
                ALOGI("set property(%s) to %s succeed.\n",
                       bt_prop, property);
        }
...
#define MB (1024 * 1024)
    ...
    if (strncmp(last_value, ptmp, strlen(ptmp))) {
        /* version info changed, update version info to WCN_BT_BUILD_VER_PROP */
        ret = property_set(WCN_BT_BUILD_VER_PROP, ptmp);
        if (!ret) {
            ALOGI("set property(%s) to (%s) succeed\n", WCN_BT_BUILD_VER_PROP, ptmp);
            strncpy(last_value, ptmp, strlen(ptmp));
	} else
            ALOGE("set property(%s) to (%s) failed,ret:%d, errno:%d, err msg:%s\n",
                  WCN_BT_BUILD_VER_PROP, ptmp, ret, errno, strerror(errno));
    }
...
static int launcher_get_wifi_patch_prefix(int chip_id, char **patch_prefix) {
    /* use malloc for alloc memory is better*/
    int chip = chip_id;
    ...
    switch (chip) {
        default:
            strncpy(g_patch_prefix, "WIFI_RAM_CODE_", strlen("WIFI_RAM_CODE_"));
        break;
    }

    ALOGI("wifi patch name pre-fix:%s\n", g_patch_prefix);
...
static int cmd_hdr_xowcn_clk_fail_count_log(struct stp_params_config *config) {
 #define MAX_XO_WCN_FAIL_COUNT 2048
     unsigned int xowcn_clk_fail_count = 0;
     unsigned int xowcn_clk_fail_count_prop = 0;
     unsigned int xowcn_clk_fail_count_update = 0;
     char ptmp[PROPERTY_VALUE_MAX] = {0};
     int ret = 0;
 
     xowcn_clk_fail_count = ioctl(g_wmt_fd, WMT_IOCTL_GET_XOWCN_CLK_FAIL_COUNT, 0);
     if (xowcn_clk_fail_count <= 0)
         return 0;
 
     ret = property_get(WCN_XO_WCN_CLK_FAIL_COUNT, ptmp, "0");
     if (0 != ret) {
         ALOGI("get property(%s) %s succeed\n", WCN_XO_WCN_CLK_FAIL_COUNT, ptmp);
         xowcn_clk_fail_count_prop = atoi(ptmp);
     } else {
         ALOGI("get property(%s) %s failed iRet:%d\n", WCN_XO_WCN_CLK_FAIL_COUNT, ptmp, ret);
     }
 
     if (xowcn_clk_fail_count_prop >= MAX_XO_WCN_FAIL_COUNT)
         return 0;
 
     xowcn_clk_fail_count_update = xowcn_clk_fail_count_prop + xowcn_clk_fail_count;
     if (xowcn_clk_fail_count_update > MAX_XO_WCN_FAIL_COUNT)
         xowcn_clk_fail_count_update = MAX_XO_WCN_FAIL_COUNT;
 
     ptmp[0] = '\0';
     snprintf(ptmp, PROPERTY_VALUE_MAX, "%d", xowcn_clk_fail_count_update);
     ret = property_set(WCN_XO_WCN_CLK_FAIL_COUNT, ptmp);
     if (0 != ret) {
...
static int launcher_set_fw_log_ctrl(void) {
    ...
    if (g_fw_debug_infor.bitmap & 0x1) {
        #ifndef VENDOR_EDIT
        //Pan.Zhang@PSW.CN.WiFi.Basic.Log.1119733, 2017/09/27,
...
/* This function is executed once when boot up.
 * It reads version of firmware patch in /vendor/firmware
 * and set to wmt driver.
 */
static int launcher_set_vendor_patch_version(int chip_id) {
    char *prefix = NULL;
    char rom_patch_prefix[16];
    char ram_patch_prefix[16];
    char wifi_patch_prefix[16];
    char patch_path[NAME_MAX + 1];
    DIR *dir = NULL;
    struct dirent* dirent = NULL;
    int status;
    ...
    /* Gget prefix of each types of patch
     * Need to copy string returned from get function, or
     * its value will be changed when another get function is
     * called.
     */
    if (launcher_get_patch_prefix(chip_id, &prefix)) {
        ALOGE("launcher_get_patch_prefix fail\n");
        return -2;
    }
    strncpy(rom_patch_prefix, prefix, strlen(prefix) + 1);

    if (launcher_get_rom_patch_prefix(chip_id, &prefix)) {
        ALOGE("launcher_get_rom_patch_prefix fail\n");
        return -2;
    }
    strncpy(ram_patch_prefix, prefix, strlen(prefix) + 1);

    if (launcher_get_wifi_patch_prefix(chip_id, &prefix)) {
        ALOGE("launcher_get_wifi_patch_prefix fail\n");
        return -2;
    }
    strncpy(wifi_patch_prefix, prefix, strlen(prefix) + 1);

    strncpy(patch_path, CUST_COMBO_PATCH_PATH, strlen(CUST_COMBO_PATCH_PATH) + 1);
    dir = opendir(patch_path);
    if (NULL == dir) {
        ALOGE("patch path cannot be opened");
        return -3;
    }
	...
	if (0 == (strncmp(dirent->d_name, rom_patch_prefix,
                strlen(rom_patch_prefix)))){
            arg.type = WMT_PATCH_TYPE_ROM;
        } else if (0 == (strncmp(dirent->d_name, ram_patch_prefix,
                strlen(ram_patch_prefix)))) {
            arg.type = WMT_PATCH_TYPE_RAM;
	} else if (0 == (strncmp(dirent->d_name, wifi_patch_prefix,
                strlen(wifi_patch_prefix)))) {
            arg.type = WMT_PATCH_TYPE_WIFI;
        } else {
            continue;
	}

        if (cp_get_version(CUST_COMBO_PATCH_PATH, dirent->d_name, arg.type,
              arg.version, chip_id) == 0) {
            strncpy(arg.file_name, dirent->d_name, strlen(dirent->d_name) + 1);
            status = ioctl(g_wmt_fd, WMT_IOCTL_SET_VENDOR_PATCH_VERSION, &arg);
            ALOGI("r:%d f:%s t:%d v:%s", status, arg.file_name, arg.type,
                arg.version);
        }
    }

    closedir(dir);
    ioctl(g_wmt_fd, WMT_IOCTL_SET_CHECK_PATCH_STATUS, WMT_CP_READY_TO_CHECK);

    /* Wait until check service finishes checking related patches.
     * If not finished in 2 mins, skip checking to continue power on chip.
     */
#if 0
    /* Skip the check process to avoid laucher is blocked by check_patch.
     */
    do {
        status = ioctl(g_wmt_fd, WMT_IOCTL_GET_CHECK_PATCH_STATUS);
        if (status == WMT_CP_CHECK_DONE) {
            return 0;
        }
        ALOGI("get check patch status fail %d", retry);
        retry++;
        usleep(300000);
    } while(retry < 80);
#endif
...
int main(int argc, char *argv[]) {
    ...
    launcher_set_vendor_patch_version(chip_id);
