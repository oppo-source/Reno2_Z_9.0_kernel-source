...
#ifdef VENDOR_EDIT
//Tao.Hui@PSW.CN.WiFi.Basic.WFD.1520404, 2018/08/20,
//Modify for  bug 1116168 wfd connet connect due to the peer side have 5g but every time work
//at 2.4G,due to next invite failed ,because next connect go freq will set 5G
//and we found serverl wfd devices have same issue ,when group save next connect tiime connect fail
//or connect name wrong
//solution:check if it is wfd devices dont save the group;
#include "p2p/p2p_i.h"
int isWfdEnable(struct wpa_supplicant *wpa_s){
	return wpa_s->global->wifi_display;
}
int isWfdDevice(u8 *wfd_dev_info){
	int device_type = 0x3;
	int type = (wfd_dev_info[0] << 8) + wfd_dev_info[1];

	int result = type & device_type;
	wpa_printf(MSG_DEBUG, "P2P: isWfdDevice type %d result %d ",type,result);
	return result;
}
int isPermitToStore(struct wpa_supplicant *wpa_s){
	...
	int i =0;
	int result = 1;
	if((NULL == p2p) || ((p2p != NULL) && (is_zero_ether_addr(p2p->peer_addr)))){
		return 1;//Oshare no need check;
	}
	wpa_printf(MSG_DEBUG, "P2P: isPermitToStore ""peer Addr " MACSTR ")", MAC2STR(p2p->peer_addr));
	struct p2p_device *peer = p2p_get_device(p2p,p2p->peer_addr);

	if(NULL == peer){
		return 1;//peer was not disvovery;
	}
	wfd_dev_info_hex = wifi_display_subelem_hex(peer->info.wfd_subelems,
	...
	if (wfd_dev_info_hex) {
		...
		for(i=0;i<wfd_dev_info_len;i++){
			wpa_printf(MSG_DEBUG, "P2P: isPermitToStore: wfd_dev_info %d",wfd_dev_info[i]);
		}

		wpa_printf(MSG_DEBUG, "P2P: isPermitToStore:wfd_dev_info_hex %s",wfd_dev_info_hex);
		if(isWfdDevice(wfd_dev_info) == 1){
			result = 0;
		}else {
			result = 1;
		}
    ...
    wpa_printf(MSG_DEBUG, "P2P: isPermitToStore result = %d", result);
	return result;
}
#endif /* VENDOR_EDIT */
					   ...
					   const u8 *go_dev_addr)
{
	...
	//or connect name wrong
	//solution:check if it is wfd devices dont save the group;
	if (isPermitToStore(wpa_s) == 0){
		wpa_printf(MSG_DEBUG, "P2P: wfd devices will not to store persist group");
		return -1;
	}
	#endif /* VENDOR_EDIT */
	...
	for (op = 0; global_op_class[op].op_class; op++) {
		...
		for (ch = o->min_chan; ch <= o->max_chan; ch += o->inc) {
			...
			if (res == ALLOWED) {
				if (reg == NULL) {
					#ifndef VENDOR_EDIT
					//Tao.Hui@PSW.CN.WiFi.Basic.Log.1015372, 2017/05/24,
					//Log reduce;
...
#ifdef VENDOR_EDIT
//Tao.Hui@PSW.CN.WiFi.Basic.P2P.1389561, 2018/05/28,
//Add for no connect ap go create at 5G.
	/* Prefer a 5 GHz channel for Oshare */
	...
	for (i = 0; i < 4; i++) {
		...
		if (!wpas_p2p_disallowed_freq(wpa_s->global, params->freq) &&
			freq_included(wpa_s, channels, params->freq) &&
			p2p_supported_freq(wpa_s->global->p2p, params->freq)) {
			wpa_printf(MSG_DEBUG, "P2P: Set GO freq by try all channels in operating class 124 "
	...
	for (i = 0; i < 4; i++) {
		...
		if (!wpas_p2p_disallowed_freq(wpa_s->global, params->freq) &&
			freq_included(wpa_s, channels, params->freq) &&
			p2p_supported_freq(wpa_s->global->p2p, params->freq)) {
			wpa_printf(MSG_DEBUG, "P2P: Set GO freq by try all channels in operating class 115 "
					"channel %d MHz", params->freq);
			goto success;
		}
	}
...
{
	//#ifndef VENDOR_EDIT
	//Tao.Hui@PSW.CN.WiFi.Basic.P2P.1292264, 2017/05/24,
	//dont do p2p formation out when neg it make auth can not  complete
	//if (wpa_s->p2p_in_provisioning && ssid->p2p_group &&
	//#else
	if (wpa_s->p2p_in_provisioning && ssid->p2p_group && (ssid == wpa_s->current_ssid) &&
	//#endif
	    eloop_cancel_timeout(wpas_p2p_group_formation_timeout,
				 wpa_s->p2pdev, NULL) > 0) {
		/**
