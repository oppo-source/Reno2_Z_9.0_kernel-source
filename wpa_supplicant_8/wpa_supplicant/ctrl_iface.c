...
//Modify for Interface for remove Wps for quickly connect Oshare.
...
static int wpas_ctrl_cmd_debug_level(const char *cmd)
{
	if (os_strcmp(cmd, "PING") == 0 ||
	    ...
	    #ifdef VENDOR_EDIT
	    //Jian.Wang@PSW.CN.WiFi.Basic.Log.1111445, 2017/09/28,
	    //add for log control
	    os_strncmp(cmd, "SIGNAL_POLL", 11) == 0||
	    os_strncmp(cmd, "PKTCNT_POLL", 11) == 0||
	    #endif /* VENDOR_EDIT */
	    os_strncmp(cmd, "STA-", 4) == 0)
					 ...
					 char *buf, size_t *resp_len)
{
	...
	} else if (os_strncmp(buf, "GET_VENDOR_SPEC", 11) == 0) {
		reply_len = wpa_supplicant_get_vendor_spec(wpa_s->current_bss, reply, reply_size);
	#endif /* VENDOR_EDIT */
#ifdef CONFIG_FILS
