			   ...
			   u8 bssid[], unsigned int id)
{
	...
	#ifndef VENDOR_EDIT
	//Shimin.Jiang@PSW.CN.WiFi.Connect.Scan.837841, 2016/08/22,
	//Removed For:[bugid 837841] QCM case id:02583150
	//analysis:Too many Aps around(like sz-air-port have 230aps), and each scan will trigger BSS removed/ added event
	//Each event cost 720 byte, hence total buffer needed 720 * 280 = 201600 bytes.
	//But the socket buffer is 229376 bytes, and reaching half the socket buffer will queue the messages into mem, and not send.
	//releate function:wpas_ctrl_iface_throttle
	//solution: android framework dont handle the msg WPA_EVENT_BSS_ADDED and WPA_EVENT_BSS_REMOVED so we can remove the notification in supplicant;
			    ...
			    const char *parameter)
{
	...
	wpa_msg_ctrl(wpa_s, MSG_INFO, WPA_EVENT_EAP_STATUS
		     ...
		     status, parameter);
	#ifdef VENDOR_EDIT
	//Shimin.Jiang@PSW.CN.WiFi.Connect.auth., 2018/3/23
	//Add for report eap failure msg to framework
	wpa_dbg(wpa_s, MSG_DEBUG,"eap status='%s' parameter='%s'", status, parameter);
	if (status != NULL && !strcmp(status, "completion")
	    && parameter != NULL && !strcmp(parameter,"failure")) {
...
void wpas_notify_select_network(struct wpa_supplicant *wpa_s)
{
	if (wpa_s->p2p_mgmt)
		return;

	wpas_hidl_notify_select_network(wpa_s);
}
void wpas_notify_ssid_temp_disabled(struct wpa_supplicant * wpa_s, char *reason) {
	if (wpa_s->p2p_mgmt)
		return;

	wpas_hidl_notify_ssid_temp_disabled(wpa_s, reason);
}
#endif /* VENDOR_EDIT */
...
//#ifdef VENDOR_EDIT
//zhaochengsheng@PSW.CN.WiFi.Basic.Custom.2223443, 2019/08/05
//add for Add for:collects wifi swap ant data
#ifdef CONFIG_MTK_SWAP_ANT_NOTIFY
void wpas_notify_bigdata_pip_payload(struct wpa_supplicant *wpa_s, u8 *payload, s32 payloadlen)
{
	wpa_printf(MSG_DEBUG, "final payloadlen: %d", payloadlen);
	if (!wpa_s || !payload || (payloadlen > 500))
		return;
	wpa_hexdump(MSG_DEBUG, "final payload:",
			payload, payloadlen);
	char *data;
	size_t len = 2*(payloadlen + 1) + 1;
	data = os_malloc(len);
	wpa_snprintf_hex(data, 2*payloadlen + 1, payload, payloadlen);
	oppo_wpas_hidl_notify_buriedpoint_data(wpa_s, data);
	os_free(data);
}
//#endif /*VENDOR_EDIT*/
#endif /* CONFIG_MTK_SWAP_ANT_NOTIFY */
