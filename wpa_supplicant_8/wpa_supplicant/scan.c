 ...
 */
//#ifndef VENDOR_EDIT
//ChuanYe.Xu@Connectivity.WiFi.Connect.select.1191818, 2017/01/13,
//modify for don't select low rssi AP
/*
#define GREAT_SNR 25
*/
//#else /* VENDOR_EDIT */
...
#define MIN(a,b) a < b ? a : b
	...
	#ifndef VENDOR_EDIT
	//Yadong.Hu@Connectivity.WiFi.Connect.select.1064666, 2016/11/10,
	//modify for select 5G wifi preferentially
...
static int wpa_scan_result_wps_compar(const void *a, const void *b)
{
	...
	#ifdef VENDOR_EDIT
	//Shimin.Jiang@PSW.CN.Connect.WPS.1051966, 2017/08/12
	//Add for wps:5G priority
	int snr_a, snr_b;
	if (wa->flags & wb->flags & WPA_SCAN_LEVEL_DBM) {
		snr_a = wa->snr > GREAT_SNR ? GREAT_SNR : wa->snr;
		snr_b = wb->snr > GREAT_SNR ? GREAT_SNR : wb->snr;
	} else {
		/* Level is not in dBm, so we can't calculate
		 * SNR. Just use raw level (units unknown). */
		snr_a = wa->level;
...
static void dump_scan_res(struct wpa_scan_results *scan_res)
{
	...
	for (i = 0; i < scan_res->num; i++) {
		...
		pos = (u8 *) (r + 1);
		#ifndef VENDOR_EDIT
		//Tao.Hui@PSW.CN.WiFi.Basic.Log.1015372, 2017/01/03,
		//Remove for reduce verbose logs
		if (r->ie_len)
