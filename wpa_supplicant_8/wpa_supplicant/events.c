					 ...
					 struct wpa_bss *bss, int debug_print)
{
	...
	int wep_ok;
	#ifdef VENDOR_EDIT
	//Shimin.Jiang@PSW.CN.WiFi.Connect.auth.749078, 2016/01/14,
	//Add for [749078: parsing failed leads to scan one ap all the time]
	struct wpa_ssid *tmp_ssid;
	#endif /* VENDOR_EDIT */
	...
	while ((ssid->proto & WPA_PROTO_RSN) && rsn_ie) {
		...
		if (wpa_parse_wpa_ie(rsn_ie, 2 + rsn_ie[1], &ie)) {
			...
			//Shimin.Jiang@PSW.CN.WiFi.Connect.auth.749078, 2016/01/14,
			//Add for [749078: parsing failed leads to scan one ap all the time]
			tmp_ssid = wpa_config_get_network(wpa_s->conf, ssid->id);
			tmp_ssid->disabled = 2;
			wpa_dbg(wpa_s, MSG_DEBUG, "   workaround solution ");
			#endif /* VENDOR_EDIT */
			if (debug_print)
				wpa_dbg(wpa_s, MSG_DEBUG,
				       ...
				       union wpa_event_data *data)
{
	...
	#ifndef VENDOR_EDIT
	//Shimin.Jiang@PSW.CN.WiFi.Connect.auth.1058482,2017/05/18
	//Delete for [FC1]-1007228:avoid ssid and bssid mismatch
#ifdef CONFIG_WAPI_SUPPORT
	...
	#ifdef VENDOR_EDIT
	//Shimin.Jiang@PSW.CN.WiFi.Connect.auth.1058482,2017/05/18
	//Add for [FC1]-1007228:avoid ssid and bssid mismatch
#ifdef CONFIG_WAPI_SUPPORT
	if (wpa_s->wpa_proto != WPA_PROTO_WAPI)
#endif
	wpa_supplicant_set_state(wpa_s, WPA_ASSOCIATED);
				 ...
				 int locally_generated)
{
    ...
    #ifndef VENDOR_EDIT
    //Yadong.Hu@PSW.CN.WiFi.Connect.auth.1058482, 2016/08/04,
    //modify for [FC1]:alert wrong key or auth timeout.
...
#ifdef CONFIG_P2P
	#ifndef VENDOR_EDIT
	//Tao.Hui@PSW.CN.WiFi.Basic.P2P.1381089, 2018/02/27,
	//Add for country code got .
	/* Ignore the avoid freq event */
...
#ifndef CONFIG_NO_STDOUT_DEBUG
{
	...
	case EVENT_BIGDATA_PIP:
		wpas_notify_bigdata_pip_payload(wpa_s,
			&(data->bigdata_pip.payload[0]),
			data->bigdata_pip.payloadlen);
		break;
