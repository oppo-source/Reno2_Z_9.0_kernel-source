...
#ifdef VENDOR_EDIT
//Botao.Cu@PSW.CN.WiFi.Connect.Scan.884561, 2016/12/05,
//Add for can't scan some APs because removed oldest unknown BSSes when more than 200
static int wpa_bss_remove_poor_rssi(struct wpa_supplicant *wpa_s, struct wpa_bss *nBss)
{
    struct wpa_bss *bss1;
    struct wpa_bss *bss2 = NULL;
    int bssCount = 0;

    //find out the most poor bss firstly
    dl_list_for_each(bss1, &wpa_s->bss, struct wpa_bss, list) {
        if (bss1->ssid_len == nBss->ssid_len
            && bss1->ssid_len > 0
            && os_memcmp(bss1->ssid, nBss->ssid, bss1->ssid_len) == 0) {
            if (bss2 == NULL || bss1->level < bss2->level) {
                bss2 = bss1;
            }
            bssCount++;
        }
    }
    wpa_dbg(wpa_s, MSG_DEBUG, "BSS count with %s is %d, current rssi=%d, lowest rssi=%d",
        wpa_ssid_txt(nBss->ssid, nBss->ssid_len), bssCount, nBss->level, bss2==NULL? 0 : bss2->level);

    //rmove the most poor bss in result list if it's rssi less than new bss
    if(bss2 != NULL && bssCount >= 20) {//Bss exists in saved list is more than 20
        if (bss2->level < nBss->level) {
            if (!wpa_bss_in_use(wpa_s, bss2)
                && !wpa_bss_known(wpa_s, bss2)) {
                wpa_bss_remove(wpa_s, bss2, __func__);
                return 0;//remove a bss and add this new bss directly
            }
        } else
            return 1; //discard this new bss
    } else
        return -1; //increase cache count to add this new bss to list
...
#ifndef VENDOR_EDIT
//Botao.Cu@PSW.CN.WiFi.Connect.Scan.884561, 2016/12/05,
//Modify for can't scan some APs because removed oldest unknown BSSes when more than 200
	...
	int ret = 0;
	if (wpa_s->num_bss + 1 > wpa_s->conf->bss_max_count) {
        ret = wpa_bss_remove_poor_rssi(wpa_s, bss);
        if (ret == -1) {
            wpa_printf(MSG_ERROR, "Increasing the MAX BSS count to %d "
                    ",because some homonymous APs less than 20 or in use, "
                    "We should normally not get here!", (int) wpa_s->num_bss + 1);
            wpa_s->conf->bss_max_count = wpa_s->num_bss + 1;
        } else if (ret == 1) {
            wpa_printf(MSG_ERROR, "Discard this BSS because homonymous APs"
            " are more than 20 and new bss is most poor!");
            free(bss);
            return NULL;
        }
	}
			     ...
			     struct os_reltime *fetch_time)
{
	...
	#ifdef VENDOR_EDIT
	//Yadong.Hu@PSW.CN.WiFi.Connect.scan.1098050, 2017/09/04,
	//add for log control
	const u8 *ie;
	u8 rsn_ie_len;
	u8 wpa_ie_len;
	...
	if (os_memcmp(wpa_s->ifname, "wlan0", 6) == 0) {
		wpa_printf(MSG_DEBUG, MACSTR " freq:%d "
			"level:%d caps:0x%x est:%u ssid:%s wpa_ie_len:%u rsn_ie_len:%u beacon_int:%u timestamp:%lu",
			MAC2STR(res->bssid), res->freq,
			res->level,
			bss->caps,
			res->est_throughput,
			wpa_ssid_txt(bss->ssid, bss->ssid_len),
			wpa_ie_len, rsn_ie_len,
			bss->beacon_int,
			bss->tsf);
	}
	#endif /* VENDOR_EDIT */
