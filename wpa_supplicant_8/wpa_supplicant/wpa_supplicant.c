...
#ifdef VENDOR_EDIT
//Yuan.Huang@PSW.CN.WiFi.Network.internet.1140781, 2017/10/30,
//Add for get_vendor_spec
...
static void wpa_supplicant_timeout(void *eloop_ctx, void *timeout_ctx)
{
	...
	//#ifndef VENDOR_EDIT
	//ChuanYe.Xu@PSW.CN.WiFi.Connect.auth.1058482, 2017/12/07,
	//Modify for bug 1071193, if WPA_4WAY_HANDSHAKE timeout, should be determined as wrong key
	/*
	...
	*/
	...
	if(wpa_s->wpa_state != WPA_4WAY_HANDSHAKE) {
		...
		wpa_msg(wpa_s, MSG_DEBUG, "4WAY_HANDSHAKE time out.");
	}
	//#endif /* VENDOR_EDIT */
	...
	#ifndef VENDOR_EDIT
	//ChuanYe.Xu@PSW.CN.WiFi.Connect.auth.1058482, 2017/12/07,
	//Remove for bug 1071193, if WPA_4WAY_HANDSHAKE timeout, should be determined as wrong key
	/*
	wpas_notify_auth_timeout(wpa_s);
	*/
	#endif /* VENDOR_EDIT */
...
#ifndef VENDOR_EDIT
//Shimin.Jiang@PSW.CN.WiFi.Connect.select.756318, 2016/07/06,
//Modify for:remove scan schedule when add connect to work
	...
	wpa_dbg(wpa_s, MSG_DEBUG, "remove scan in work and excute connect now");
#endif /* VENDOR_EDIT */
}
...
static void wpas_start_assoc_cb(struct wpa_radio_work *work, int deinit)
{
	...
	#ifdef VENDOR_EDIT
	//Tao.Hui@PSW.CN.WiFi.Basic.Log.1015372, 2016/08/01,
	//Add for log of wifi MAC address
	wpa_dbg(wpa_s, MSG_INFO, "OWN MAC adrress: " MACSTR, MAC2STR(wpa_s->own_addr));
	#endif /* VENDOR_EDIT */
	wpa_sm_set_assoc_wpa_ie(wpa_s->wpa, NULL, 0);
				   ...
				   struct wpa_ssid *ssid)
{
	...
	#ifdef VENDOR_EDIT
	//Chuanye.Xu@PSW.CN.WiFi.Connect.auth.1058482, 2017/10/25,
	//add for [FC1]:alert wrong key or auth timeout.
	wpas_notify_select_network(wpa_s);
	#endif /* VENDOR_EDIT */
	if (ssid && ssid == wpa_s->current_ssid && wpa_s->current_ssid &&
	    wpa_s->wpa_state >= WPA_AUTHENTICATING) {
...
static void radio_start_next_work(void *eloop_ctx, void *timeout_ctx)
{
	...
	if (!(wpa_s &&
		...
		if (wpa_s && wpa_s->radio->external_scan_running) {
			#ifndef VENDOR_EDIT
			//Shimin.Jiang@PSW.CN.WiFi.Connect.select.1275298, 2018/2/20
			//Modify for connect work delayed by scan
			wpa_printf(MSG_DEBUG, "Delay radio work start until externally triggered scan completes");
			return;
			#else
			if (radio_work_is_connect(work)) {
				 wpa_drv_abort_scan(wpa_s, 0);
				 wpa_printf(MSG_DEBUG, "abort scan and connect!");
			}else {
				wpa_printf(MSG_DEBUG, "Delay radio work start until externally triggered scan completes");
				return;
			}
...
void wpas_connection_failed(struct wpa_supplicant *wpa_s, const u8 *bssid)
{
	...
	default:
		#ifndef VENDOR_EDIT
		//Yadong.Hu@PSW.CN.WiFi.Connect.auth.1058482, 2016/08/23,
		//modify for [FC1]:alert wrong key or auth timeout,shorten the scan time interval.
		timeout = 10000;
		#else /* VENDOR_EDIT */
		timeout = 5000;
		#endif /* VENDOR_EDIT */
...
#endif /* CONFIG_P2P */
#ifndef VENDOR_EDIT
//Shimin.Jiang@PSW.CN.WiFi.Connect.select.1064337, 2016/07/07,
//Modify for [FC5]:optimize auth failure time
/*
...
*/
	...
	//#ifdef VENDOR_EDIT
	//ChuanYe.Xu@PSW.CN.WiFi.Connect.auth.1058482, 2017/12/31,
	//Add for alert wrong key or auth timeout
	wpas_notify_ssid_temp_disabled(wpa_s, reason);
	//#endif /* VENDOR_EDIT */
}
...
void wpas_request_disconnection(struct wpa_supplicant *wpa_s)
{
	...
	eloop_cancel_timeout(wpas_network_reenabled, wpa_s, NULL);
#ifdef VENDOR_EDIT
	//Shimin.Jiang@PSW.CN.WiFi.Connect.Scan.815906, 2016/06/13, Add for DUT can not scan
	if (os_strcmp(wpa_s->ifname, "wlan0") == 0) {
		wpa_printf(MSG_DEBUG, "WPA: Remove connect work to avoid race condition");
		wpas_connect_work_done(wpa_s);
	}
#endif /* VENDOR_EDIT */
}
...
//Yuan.Huang@PSW.CN.WiFi.Network.internet.1140781, 2017/10/30,
//Add for get_vendor_spec
int wpa_supplicant_get_vendor_spec(struct wpa_bss *bss, char *buf, size_t buflen)
{
	char *vendor_spec = buf;
	struct wpabuf *wps_ie;
	const u8 *end, *pos;
	int ret = 0;

	wpa_printf(MSG_DEBUG, "wpa_supplicant_get_vendor_spec start!");
	if (NULL != bss) {
		wps_ie = wpa_bss_get_vendor_ie_multi(bss, WPS_IE_VENDOR_TYPE);
		if (NULL == wps_ie) {
			wpa_printf(MSG_DEBUG, "wpa_supplicant_get_vendor_spec no wps ie from probe response, try beacon.");
			wps_ie = wpa_bss_get_vendor_ie_multi_beacon(bss, WPS_IE_VENDOR_TYPE);
		}
	} else {
		wpa_printf(MSG_DEBUG, "wpa_supplicant_get_vendor_spec current_bss==NULL!");
		return -1;
	}
	if (NULL != wps_ie) {
	    ...
	    if (wps_parse_msg(wps_ie, &attr) < 0) {
	        wpa_printf(MSG_DEBUG, "wpa_supplicant_get_vendor_spec failed to parse wps ie!");
	        wpabuf_free(wps_ie);
	        goto chipset;
	    }
	    if (NULL != attr.manufacturer && attr.manufacturer_len > 0
	            && NULL != attr.model_name && attr.model_name_len > 0
	            && NULL != attr.model_number && attr.model_number_len > 0) {
	        char manufacturer[512];
	        char model_name[512];
	        char model_number[512];
	        //manufacturer
	        strncpy(manufacturer, (char*)attr.manufacturer, attr.manufacturer_len);
	        manufacturer[attr.manufacturer_len] = '\0';
	        //model_name
	        strncpy(model_name, (char*)attr.model_name, attr.model_name_len);
	        model_name[attr.model_name_len] = '\0';
	        //model_number
	        strncpy(model_number, (char*)attr.model_number, attr.model_number_len);
	        model_number[attr.model_number_len] = '\0';

	        ret = os_snprintf(vendor_spec, buflen, "Manufacturer:%s, Model:%s, Model number:%s ",
	                manufacturer, model_name, model_number);

	        wpa_printf(MSG_DEBUG, "wpa_supplicant_get_vendor_spec from wps_ie ret=%d, manufacturer_len=%d, model_name_len=%d, model_number_len=%d\n",
	                ret, attr.manufacturer_len, attr.model_name_len, attr.model_number_len);
	        wpa_hexdump(MSG_DEBUG, "dump manufacturer", attr.manufacturer, attr.manufacturer_len);
	        wpa_hexdump(MSG_DEBUG, "dump model_name", attr.model_name, attr.model_name_len);
	        wpa_hexdump(MSG_DEBUG, "dump model_number", attr.model_number, attr.model_number_len);
	        wpa_hexdump(MSG_DEBUG, "dump vendor_spec with wps ie", vendor_spec, ret);
	        wpa_printf(MSG_DEBUG, "wpa_supplicant_get_vendor_spec wps ie done! vendor_spec=%s", vendor_spec);
	    } else {
	        wpa_printf(MSG_DEBUG, "wpa_supplicant_get_vendor_spec no manufacturer or model_name!");
	    }
	    wpabuf_free(wps_ie);
	} else {
	    wpa_printf(MSG_DEBUG, "wpa_supplicant_get_vendor_spec no wps ie!");
	}

chipset:
    //deal with chipset vendor
	pos = (const u8 *) (bss + 1);
	end = pos + bss->ie_len;

	while (end - pos > 4) {
		if (2 + pos[1] > end - pos) break;

		if (pos[0] == WLAN_EID_VENDOR_SPECIFIC && pos[1] >= 4 &&
		        OUI_MICROSOFT != WPA_GET_BE24(&pos[2])) {
		    ret += os_snprintf(vendor_spec + ret, buflen - ret, "Chipset OUI:%02X-%02X-%02X, Value:", pos[2], pos[3], pos[4]);
		    ret += wpa_snprintf_hex(vendor_spec + ret, buflen - ret, pos + 5, (pos[1] - 3) > 10 ? 10 : (pos[1] - 3));
		    vendor_spec[ret] = '\0';
		    wpa_hexdump(MSG_DEBUG, "dump vendor_spec with chipset info", vendor_spec, ret);
		    break;
		}
		pos += 2 + pos[1];
	}
	wpa_printf(MSG_DEBUG, "wpa_supplicant_get_vendor_spec %s, ret=%d\n", buf, ret);
	return ret;
}
int wpa_supplicant_get_bss_load(struct wpa_bss *bss, char *buf, size_t buflen)
{
	char *bss_load = buf;
	const u8 *end, *pos;
	int ret = 0;

	wpa_printf(MSG_DEBUG, "wpa_supplicant_get_bss_load start!");

	if (bss == NULL) {
		wpa_printf(MSG_DEBUG, "wpa_supplicant_get_bss_load current_bss==NULL!");
		return -1;
	}
	...
	while (end - pos > 4) {
		...
		if (pos[0] == WLAN_EID_BSS_LOAD) {
		    ret += os_snprintf(bss_load, buflen, "Station Count:%d", pos[3] * 256 + pos[2]);
		    break;
		}
		pos += 2 + pos[1];
	}
	wpa_printf(MSG_DEBUG, "wpa_supplicant_get_bss_load %s, ret=%d\n", buf, ret);
	return ret;
}
int wpa_supplicant_get_vht_operation(struct wpa_bss *bss, char *buf, size_t buflen)
{
	char *vht_operation = buf;
	const u8 *end, *pos;
	int ret = 0;

	wpa_printf(MSG_DEBUG, "wpa_supplicant_get_vht_operation start!");

	if (bss == NULL) {
		wpa_printf(MSG_DEBUG, "wpa_supplicant_get_vht_operations current_bss==NULL!");
		return -1;
	}

	//deal with vht operation
	pos = (const u8 *) (bss + 1);
	end = pos + bss->ie_len;
	...
	while (end - pos > 4) {
		...
		if (pos[0] == WLAN_EID_VHT_OPERATION) {
		    ret += os_snprintf(vht_operation, buflen, "VHT operation Channel Width:%d", pos[2]);
		    break;
		}
		pos += 2 + pos[1];
	}
	wpa_printf(MSG_DEBUG, "wpa_supplicant_get_vht_operation %s, ret=%d\n", buf, ret);
	return ret;
}
#endif /* VENDOR_EDIT */

#ifdef VENDOR_EDIT
//Chuanye.Xu@PSW.CN.WiFi.Connect.select.1065686, 2017/11/17,
//Add for can not connect eight emoticons or Chinese SSID or non-UTF-8 SSID
int tryGetSsidByBssid(struct wpa_ssid *wpa_ssid, struct wpa_supplicant *wpa_s, u8 *newest_ssid_quoted) {
	struct wpa_bss *bss;
	u8 sameBssidNum = 0;
	u8 ssid_quoted[35];
	u64 newest_tsf = 0L;
	if (!wpa_supplicant_filter_bssid_match(wpa_s, wpa_ssid->bssid))
		return -1;
	dl_list_for_each(bss, &wpa_s->bss, struct wpa_bss, list) {
		//wpa_printf(MSG_DEBUG, "update_network ssid:%s, bssid:" MACSTR, bss->ssid, MAC2STR(bss->bssid));
		if (os_memcmp(bss->bssid, wpa_ssid->bssid, ETH_ALEN) == 0 && os_strlen((char *)(bss->ssid)) > 0) {
			os_memset(ssid_quoted, 0, sizeof(ssid_quoted));
			os_snprintf((char*)ssid_quoted, sizeof(ssid_quoted), "%s", bss->ssid);
			ssid_quoted[bss->ssid_len] = '\0';
			wpa_printf(MSG_DEBUG, "Setting ssid:%s for bss:" MACSTR, ssid_quoted, MAC2STR(bss->bssid));
			sameBssidNum++;
			if (bss->tsf > newest_tsf) {
				newest_tsf = bss->tsf;
				os_memcpy(newest_ssid_quoted, ssid_quoted, sizeof(ssid_quoted));
			}
		}
	}
	if (sameBssidNum > 1) {
		wpa_printf(MSG_DEBUG, "there are %d same BSSID!", sameBssidNum);
		return 0;
	} else if (sameBssidNum == 1) {
		return 0;
	} else {
		return -1;
	}
}
int hasSetSsid(struct wpa_ssid *wpa_ssid) {
	char t_bssid[18];
	os_snprintf(t_bssid, sizeof(t_bssid), MACSTR, MAC2STR(wpa_ssid->bssid));
	if (wpa_ssid->ssid != NULL && os_strcmp(t_bssid, "00:00:00:00:00:00") != 0) {
		return 1;
	} else {
		return 0;
	}
