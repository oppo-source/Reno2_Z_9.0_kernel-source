				     ...
				     union wps_event_data *data)
{
	...
	case WPS_EV_PBC_OVERLAP:
		#ifdef  VENDOR_EDIT
		//Shimin.Jiang@PSW.CN.Connect.WPS.1051966, 2017/08/23
		//Add for wps :ap detect overlap
		wpa_printf(MSG_DEBUG, "WPS: WPS_EV_PBC_OVERLAP  by ap");
		wpa_msg(wpa_s, MSG_INFO, WPS_EVENT_OVERLAP "PBC overlap detected by ap!");
			      ...
			      struct wpa_bss *selected, struct wpa_ssid *ssid)
{
	...
	//Add for wps : igonre overlap
	if (ret){
		wpa_msg(wpa_s,MSG_INFO, "WPS:PBC overlap detected,but we still try to connect!");
	}
	ret =  0;
	#endif /*VENDOR_EDIT*/
