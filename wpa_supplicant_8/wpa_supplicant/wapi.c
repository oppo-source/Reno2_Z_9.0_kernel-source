...
#ifdef VENDOR_EDIT
//Shimin.Jiang@PSW.CN.WiFi.Connect.auth.1244116,2018/2/2
//Add for notify framwork
#include "notify.h"
#endif
#ifndef ETH_P_WAI
...
static void wapi_deauthenticate_helper(void *ctx, int reason_code)
{
	...
	int wpa_state = wapi_get_state_helper(ctx);
	if (ssid && ssid->key_mgmt == WPA_KEY_MGMT_WAPI_CERT){
		if (wpa_state == WPA_ASSOCIATING) {
...
#else
	wpa_printf(MSG_DEBUG, "[WAPI] wapi_init");
#endif
