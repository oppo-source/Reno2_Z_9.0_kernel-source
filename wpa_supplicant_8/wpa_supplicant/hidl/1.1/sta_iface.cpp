...
#ifdef VENDOR_EDIT
//Botao.Cu@PSW.CN.WiFi.Connect.select.1063227, 2017/01/04,
//Add for make connecting faster
#include "scan.h"
#endif /* VENDOR_EDIT */
}
...
#ifdef VENDOR_EDIT
//Tao.Hui@PSW.CN.WiFi.Basic.P2P.1069764, 2017/11/02,
//Add for Hidl interface for mtk Oshare Mode.
constexpr char kSetOshareMode[] = "OSHAREMOD ";
using vendor::oppo::hardware::wifi::supplicant::V1_0::IOppoSupplicantStaIface;
uint8_t convertHidlOshareToInternal(
    IOppoSupplicantStaIface::OshareMode mode)
{
	switch (mode) {
	case IOppoSupplicantStaIface::OshareMode::DISABLED:
		return 0;
	case IOppoSupplicantStaIface::OshareMode::OPEN:
		return 1;
	};
	WPA_ASSERT(false);
}
...
#ifdef VENDOR_EDIT
//Chuanye.Xu@PSW.CN.Wifi.Connect.AutoConnect.872537, 2017/11/11,
//Add for oppo wifi reconnect
Return<void> StaIface::manuReassociate(manuReassociate_cb _hidl_cb)
{
	return validateAndCall(
	    this, SupplicantStatusCode::FAILURE_IFACE_INVALID,
	    &StaIface::manuReassociateInternal, _hidl_cb);
}
#endif /* VENDOR_EDIT */

#ifdef VENDOR_EDIT
//Yuan.Huang@PSW.CN.WiFi.Network.1140781, 2017/10/30,
//Add for data stall detection:get AP vendor info
Return<void> StaIface::doStringCommand(
    const hidl_string &cmd, doStringCommand_cb _hidl_cb)
{
	return validateAndCall(
	    this, SupplicantStatusCode::FAILURE_IFACE_INVALID,
	    &StaIface::doStringCommandInternal, _hidl_cb, cmd);
}
#endif /* VENDOR_EDIT */
...
//Chuanye.Xu@PSW.CN.Wifi.Connect.AutoConnect.872537, 2017/11/11,
//Add for oppo wifi reconnect
SupplicantStatus StaIface::manuReassociateInternal()
{
	struct wpa_supplicant *wpa_s = retrieveIfacePtr();
	if (wpa_s->wpa_state == WPA_INTERFACE_DISABLED) {
		return {SupplicantStatusCode::FAILURE_IFACE_DISABLED, ""};
	...
	if (wpa_s->wpa_state >= WPA_ASSOCIATING)
		wpa_printf(MSG_DEBUG, "wpa_s is already in associating/ed state,do nothing!");
	else
		wpas_request_connection(wpa_s);
	return {SupplicantStatusCode::SUCCESS, ""};
}
...
//Yuan.Huang@PSW.CN.WiFi.Network.1140781, 2017/10/30,
//Add for data stall detection:get AP vendor info
std::pair<SupplicantStatus, std::string> StaIface::doStringCommandInternal(
    const std::string &cmd)
{
	wpa_printf(MSG_DEBUG, "doStringCommandInternal");
	struct wpa_supplicant *wpa_s = retrieveIfacePtr();
	char buff[1024] = {0};
	if (cmd.compare("GET_VENDOR_SPEC") == 0) {
	    wpa_supplicant_get_vendor_spec(wpa_s->current_bss, buff, 1024);
	}
	if (cmd.compare("GET_BSS_LOAD") == 0) {
		wpa_supplicant_get_bss_load(wpa_s->current_bss, buff, 1024);
	}
	if (cmd.compare("GET_VHT_OPERATION") == 0) {
		wpa_supplicant_get_vht_operation(wpa_s->current_bss, buff, 1024);
	}
	std::string result(buff);
	return {{SupplicantStatusCode::SUCCESS, ""}, result};
}
#endif /* VENDOR_EDIT */
...
SupplicantStatus StaIface::reconnectInternal()
{
	...
	//Botao.Cu@PSW.CN.WiFi.Connect.select.1063227, 2017/01/04,
	//Add for make connecting faster
	if (wpa_s->scanning && wpa_s->scan_res_handler == scan_only_handler) {
		wpa_printf(MSG_DEBUG, "RECONNECT when scan only on going, clear scan_res_handler");
...
//Tao.Hui@PSW.CN.WiFi.Basic.P2P.1069764, 2017/11/02,
//Add for Hidl interface for mtk Oshare Mode.
Return<void> StaIface::setOshareMod(
     IOppoSupplicantStaIface::OshareMode mode, setOshareMod_cb _hidl_cb)
{
	return validateAndCall(
	    this, SupplicantStatusCode::FAILURE_IFACE_INVALID,
	    &StaIface::setOshareModInternal,_hidl_cb, mode);
}
SupplicantStatus StaIface::setOshareModInternal(
    IOppoSupplicantStaIface::OshareMode mode)
{
	struct wpa_supplicant* wpa_s = retrieveIfacePtr();

	uint8_t mode_internal = convertHidlOshareToInternal(mode);
	const std::string cmd_str =
	    kSetOshareMode + std::to_string(mode_internal);
	std::vector<char> cmd(
