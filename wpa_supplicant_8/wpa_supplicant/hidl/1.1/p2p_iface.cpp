...
#ifdef VENDOR_EDIT
//Tao.Hui@PSW.CN.WiFi.Basic.P2P.1069764, 2018/01/14,
//Add for Interface for remove Wps for quickly connect Oshare.
...
#ifdef VENDOR_EDIT
//Jian.Wang@PSW.CN.WiFi.Basic.P2P.1069764, 2017/11/02,
//Add for Hidl interface for p2p fast connection.
Return<void> P2pIface::p2pConnect(
    const hidl_array<uint8_t, 6>& peer_address,
    ISupplicantP2pIface::WpsProvisionMethod provision_method,
    const hidl_string& pre_selected_pin, bool join_existing_group,
    bool persistent, uint32_t go_intent, uint32_t freq, connect_cb _hidl_cb)
{
	return validateAndCall(
	    this, SupplicantStatusCode::FAILURE_IFACE_INVALID,
	    &P2pIface::connectFreqInternal, _hidl_cb, peer_address,
	    provision_method, pre_selected_pin, join_existing_group, persistent,
	    go_intent, freq);
}

Return<void> P2pIface::p2pFind(uint32_t timeout_in_sec, uint32_t freq, find_cb _hidl_cb)
{
	return validateAndCall(
	    this, SupplicantStatusCode::FAILURE_IFACE_INVALID,
	    &P2pIface::findFreqInternal, _hidl_cb, timeout_in_sec, freq);
}

std::pair<SupplicantStatus, std::string> P2pIface::connectFreqInternal(
    const std::array<uint8_t, 6>& peer_address,
    ISupplicantP2pIface::WpsProvisionMethod provision_method,
    const std::string& pre_selected_pin, bool join_existing_group,
    bool persistent, uint32_t go_intent, uint32_t freq)
{
	wpa_printf(
				    MSG_ERROR, "connectFreqInternal freq: %d",
				    freq);
	struct wpa_supplicant* wpa_s = retrieveIfacePtr();
	if (go_intent > 15) {
		return {{SupplicantStatusCode::FAILURE_ARGS_INVALID, ""}, {}};
	...
	int new_pin = wpas_p2p_connect(
	    wpa_s, peer_address.data(), pin, wps_method,
	    persistent, false, join_existing_group, false, go_intent_signed, freq, 0, -1,
...
SupplicantStatus P2pIface::findFreqInternal(uint32_t timeout_in_sec, uint32_t freq)
{
	wpa_printf(
				    MSG_ERROR, "findFreqInternal freq: %d",
				    freq);
	struct wpa_supplicant* wpa_s = retrieveIfacePtr();
	if (wpa_s->wpa_state == WPA_INTERFACE_DISABLED) {
		return {SupplicantStatusCode::FAILURE_IFACE_DISABLED, ""};
...
Return<void> P2pIface::setOperChannel(
    uint32_t op_channel, uint32_t op_reg_class, uint32_t cfg_op_channel, setOperChannel_cb _hidl_cb)
{
	return validateAndCall(
	    this, SupplicantStatusCode::FAILURE_IFACE_INVALID,
	    &P2pIface::setOperChannelInternal, _hidl_cb,
	    op_channel, op_reg_class, cfg_op_channel);
}

SupplicantStatus P2pIface::setOperChannelInternal(
    uint32_t op_channel, uint32_t op_reg_class, uint32_t cfg_op_channel)
{
	struct wpa_supplicant* wpa_s = retrieveIfacePtr();
	if (p2p_set_oper_channel(
		wpa_s->global->p2p, op_reg_class, op_channel, cfg_op_channel)) {
		return {SupplicantStatusCode::FAILURE_UNKNOWN, ""};
	}
	return {SupplicantStatusCode::SUCCESS, ""};
}
...
//Tao.Hui@PSW.CN.WiFi.Basic.P2P.1069764, 2018/01/14,
//Add for Interface for remove Wps for quickly connect Oshare.
Return<void> P2pIface::setNetworkVariable(
	uint32_t netId, const hidl_string& name, const hidl_string& value,setNetworkVariable_cb _hidl_cb)
{
	return validateAndCall(
		this, SupplicantStatusCode::FAILURE_IFACE_INVALID,
		&P2pIface::setNetworkVariableInternal, _hidl_cb,
		netId, name, value);
}

SupplicantStatus P2pIface::setNetworkVariableInternal(
	uint32_t netId, const std::string&  name, const std::string&  value)
{
	struct wpa_supplicant* wpa_s = retrieveIfacePtr();
	char spac[] = " ";
	const std::string cmd_str =
	    std::to_string(netId) + spac + name + spac + value;
	std::vector<char> cmd(
	    cmd_str.c_str(), cmd_str.c_str() + cmd_str.size() + 1);
	if (wpa_supplicant_ctrl_iface_set_network(wpa_s,cmd.data())){
		return {SupplicantStatusCode::FAILURE_UNKNOWN, ""};
	}
	return {SupplicantStatusCode::SUCCESS, ""};
}

Return<void> P2pIface::addP2pNetwork(addP2pNetwork_cb _hidl_cb)
{
	return validateAndCall(
		this, SupplicantStatusCode::FAILURE_IFACE_INVALID,
		&P2pIface::addP2pNetworkInternal, _hidl_cb);
}

std::pair<SupplicantStatus, uint32_t>P2pIface::addP2pNetworkInternal()
{
	struct wpa_supplicant* wpa_s = retrieveIfacePtr();
	struct wpa_ssid* ssid = wpa_supplicant_add_network(wpa_s);
	int netId = -1;
	if (!ssid) {
		return {{SupplicantStatusCode::FAILURE_UNKNOWN, ""}, netId};
	}
	netId = ssid->id;
	return {{SupplicantStatusCode::SUCCESS, ""}, netId};
}
