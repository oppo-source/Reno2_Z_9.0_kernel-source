...
#include <vendor/oppo/hardware/wifi/supplicant/1.0/IOppoSupplicantP2pIface.h>
#endif /* VENDOR_EDIT */
...
#ifndef VENDOR_EDIT
//Jian.Wang@PSW.CN.WiFi.Basic.P2P.1069764, 2017/11/02,
//Modify for Hidl interface for p2p fast connection.
class P2pIface : public ISupplicantP2pIface
#else /* VENDOR_EDIT */
class P2pIface : public vendor::oppo::hardware::wifi::supplicant::V1_0::IOppoSupplicantP2pIface
#endif /* VENDOR_EDIT */
...
public:
	...
	//Jian.Wang@PSW.CN.WiFi.Basic.P2P.1069764, 2017/11/02,
	//Add for Hidl interface for p2p fast connection.
	Return<void> p2pConnect(
	    const hidl_array<uint8_t, 6>& peer_address,
	    ISupplicantP2pIface::WpsProvisionMethod provision_method,
	    const hidl_string& pre_selected_pin, bool join_existing_group,
	    bool persistent, uint32_t go_intent, uint32_t freq, p2pConnect_cb _hidl_cb) override;
	Return<void> p2pFind(uint32_t timeout_in_sec, uint32_t freq, p2pFind_cb _hidl_cb) override;
	Return<void> setOperChannel(
			uint32_t op_channel, uint32_t op_reg_class,
			uint32_t cfg_op_channel, setOperChannel_cb _hidl_cb) override;
	Return<void> setNetworkVariable(
			uint32_t netId, const hidl_string& name, const hidl_string& value ,setNetworkVariable_cb _hidl_cb) override;
	Return<void> addP2pNetwork(addP2pNetwork_cb _hidl_cb) override;
#endif /* VENDOR_EDIT */
...
private:
	...
	//Jian.Wang@PSW.CN.WiFi.Basic.P2P.1069764, 2017/11/02,
	//Add for Hidl interface for p2p fast connection.
	std::pair<SupplicantStatus, std::string> connectFreqInternal(
		const std::array<uint8_t, 6>& peer_address,
		ISupplicantP2pIface::WpsProvisionMethod provision_method,
		const std::string& pre_selected_pin, bool join_existing_group,
		bool persistent, uint32_t go_intent, uint32_t freq);
	SupplicantStatus findFreqInternal(uint32_t timeout_in_sec, uint32_t freq);
	SupplicantStatus setOperChannelInternal(
		uint32_t op_channel, uint32_t op_reg_class, uint32_t cfg_op_channel);
	SupplicantStatus setNetworkVariableInternal(
		uint32_t netId, const std::string& name, const std::string& value);
	std::pair<SupplicantStatus, uint32_t> addP2pNetworkInternal();
#endif /* VENDOR_EDIT */
