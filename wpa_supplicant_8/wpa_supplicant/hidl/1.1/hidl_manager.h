...
public:
	...
	//Chuanye.Xu@PSW.CN.WiFi.Connect.auth.1058482, 2017/10/25,
	//add for [FC1]:alert wrong key or auth timeout.
	void notifySelectNetwork(struct wpa_supplicant *wpa_s);
	void notifySsidTempDisabled(struct wpa_supplicant * wpa_s, char *reason);
#endif /* VENDOR_EDIT */
    ...
    */
	...
	int getStaIfaceHidlObjectByIfname(
	    const std::string &ifname,
	    android::sp<IOppoSupplicantStaIface> *iface_object);
#endif /* VENDOR_EDIT */
...
private:
	...
	//Chuanye.Xu@PSW.CN.WiFi.Connect.auth.1058482, 2017/10/25,
	//add for [FC1]:alert wrong key or auth timeout.
	void callWithEachOppoStaIfaceCallback(
	    const std::string &ifname,
	    const std::function<android::hardware::Return<void>(
		android::sp<IOppoSupplicantStaIfaceCallback>)> &method);
#endif /* VENDOR_EDIT */
