...
void wpas_hidl_notify_select_network(struct wpa_supplicant *wpa_s)
{
	if (!wpa_s)
		return;

	wpa_printf(MSG_DEBUG, "Notifying select network to hidl control");

	HidlManager *hidl_manager = HidlManager::getInstance();
	if (!hidl_manager)
		return;

	hidl_manager->notifySelectNetwork(wpa_s);
}
void wpas_hidl_notify_ssid_temp_disabled(struct wpa_supplicant * wpa_s, char *reason)
{
	if (!wpa_s)
		return;

	wpa_printf(MSG_DEBUG, "Notifying ssid temp disabled to hidl control");

	HidlManager *hidl_manager = HidlManager::getInstance();
	if (!hidl_manager)
		return;

	hidl_manager->notifySsidTempDisabled(wpa_s, reason);
}
#endif /* VENDOR_EDIT */
