...
//add for [FC1]:alert wrong key or auth timeout.
template <class CallbackType>
void callWithEachOppoIfaceCallback(
    const std::string &ifname,
    const std::function<android::hardware::Return<
	void>(android::sp<IOppoSupplicantStaIfaceCallback>)> &method,
	...
	&callbacks_map)
{
	...
	const auto &iface_callback_list = iface_callback_map_iter->second;
	for (const auto &callback : iface_callback_list) {
		android::sp<IOppoSupplicantStaIfaceCallback> oppoCallback =
			IOppoSupplicantStaIfaceCallback::castFrom(callback);
		if (oppoCallback != nullptr &&
		    !method(oppoCallback).isOk()) {
			wpa_printf(
				MSG_ERROR,
...
//Chuanye.Xu@PSW.CN.WiFi.Connect.auth.1058482, 2017/10/25,
//add for [FC1]:alert wrong key or auth timeout.
void HidlManager::notifySelectNetwork(struct wpa_supplicant *wpa_s)
{
	if (!wpa_s)
		return;
	...
	callWithEachOppoStaIfaceCallback(
	    wpa_s->ifname,
	    std::bind(
		&IOppoSupplicantStaIfaceCallback::onSelectNetwork,
		std::placeholders::_1, bssid));
}
void HidlManager::notifySsidTempDisabled(struct wpa_supplicant * wpa_s, char *reason)
{
	if (!wpa_s || !reason)
		return;
	...
	callWithEachOppoStaIfaceCallback(
	    wpa_s->ifname,
	    std::bind(
		&IOppoSupplicantStaIfaceCallback::onSsidTempDisabled,
		std::placeholders::_1, reason));
}
#endif /* VENDOR_EDIT */
...
#else /* VENDOR_EDIT */
int HidlManager::getStaIfaceHidlObjectByIfname(
    const std::string &ifname, android::sp<IOppoSupplicantStaIface> *iface_object)
#endif /* VENDOR_EDIT */
{
	if (ifname.empty() || !iface_object)
		return 1;
...
//add for [FC1]:alert wrong key or auth timeout.
/**
 * Helper fucntion to invoke the provided oppo callback method on all the
 * registered iface callback hidl objects for the specified
 * |ifname|.
 *
 * @param ifname Name of the corresponding interface.
 ...
 */
void HidlManager::callWithEachOppoStaIfaceCallback(
    const std::string &ifname,
    const std::function<
	Return<void>(android::sp<IOppoSupplicantStaIfaceCallback>)> &method)
{
	callWithEachOppoIfaceCallback(ifname, method,
