...
//Modify for can not connect eight emoticons or Chinese SSID or non-UTF-8 SSID
/*
...
*/
#else /* VENDOR_EDIT */
	if (hasSetSsid(wpa_ssid)) {
		wpa_printf(MSG_DEBUG, "SSID already set, no need to override.");
	} else if (setByteArrayFieldAndResetState(
		ssid.data(), ssid.size(), &(wpa_ssid->ssid),
		&(wpa_ssid->ssid_len), "ssid")) {
		return {SupplicantStatusCode::FAILURE_UNKNOWN, ""};
	}
    ...
    const std::array<uint8_t, 6> &bssid)
{
	...
	if (os_memcmp(bssid.data(), kZeroBssid, ETH_ALEN) == 0) {
		...
		u8 newest_ssid_quoted[35];
		os_memset(newest_ssid_quoted, 0, sizeof(newest_ssid_quoted));
		if (tryGetSsidByBssid(wpa_ssid, retrieveIfacePtr(), newest_ssid_quoted) == 0) {
			if (setByteArrayFieldAndResetState(
				newest_ssid_quoted, os_strlen((char *)newest_ssid_quoted), &(wpa_ssid->ssid),
				&(wpa_ssid->ssid_len), "ssid")) {
				wpa_printf(MSG_DEBUG, "Failed to set ssid by bssid");
			} else if (wpa_ssid->passphrase) {
				wpa_config_update_psk(wpa_ssid);
			}
		}
		#endif /* VENDOR_EDIT */
    ...
    const char *value, char **to_update_field, const char *hexdump_prefix)
{
	...
	*to_update_field = dup_binstr(value, value_len);
	...
	#ifndef VENDOR_EDIT
	//Chuanye.Xu@PSW.CN.WiFi.Connect.disconnect.990474, 2017/11/25,
	//Modify for some app(for example wifisettings) save id_str, but ssid is null
	/*
	resetInternalStateAfterParamsUpdate();
	*/
	#else /* VENDOR_EDIT */
	if (os_strcmp("id_str", hexdump_prefix) != 0) {
	    resetInternalStateAfterParamsUpdate();
	}
	#endif /* VENDOR_EDIT */
	return 0;
