...
#ifdef VENDOR_EDIT
//Tao.Hui@PSW.CN.WiFi.Basic.WFD.1451892, 2018/07/02,
//ADD for wfd can not connect due to peer give a wrong oper_freq that peer not include.
int p2p_peer_channel_include_in(struct p2p_data *p2p, struct p2p_device *dev){
	u8 op_class = 0, op_channel = 0;
	if (dev->oper_freq > 0 &&
		p2p_freq_to_channel(dev->oper_freq,
				&op_class, &op_channel) == 0 &&
		p2p_channels_includes(&dev->channels, op_class, op_channel)) {
		p2p_dbg(p2p, "peer operating channel (op_class %u channel %u) include in peer list",op_class, op_channel);
		return 1;
	}
	p2p_dbg(p2p, "peer error,peer operating channel (op_class %u channel %u) not in the peer list",
		op_class, op_channel);
			 ...
			 struct p2p_device *dev, struct p2p_message *msg)
{
	...
	//Tao.Hui@PSW.CN.WiFi.Basic.WFD.1451892, 2018/07/02,
	//ADD for wfd can not connect due to peer give a wrong oper_freq that peer not include.
	if (p2p_peer_channel_include_in(p2p, dev) == 0){
		return;
	}
	#endif /* VENDOR_EDIT */
