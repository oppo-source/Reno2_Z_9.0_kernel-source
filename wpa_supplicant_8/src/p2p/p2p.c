...
#ifdef VENDOR_EDIT
//Guotian.Wu@PSW.CN.WiFi.Basic.P2P.1408501 2018/08/07
//Modify for confirm p2p state not idle so can continue find
static void confirm_p2p_state(void *eloop_ctx, void *timeout_ctx);
#endif /* VENDOR_EDIT */
/*
...
#ifdef VENDOR_EDIT
//Laixin@PSW.CN.WiFi.Basic.P2P.1408501, 2018/06/13
//Add for: avoid driver busy doing wlan scan;1194041
#define P2P_SCAN_REQ_FAILED_AVOID_DRIVER_BUSY_TIMEOUT 2
#endif /* VENDOR_EDIT */
...
static void p2p_search(struct p2p_data *p2p)
{
	...
	if (p2p->find_pending_full &&
		...
		//#ifdef VENDOR_EDIT
		//Longbo.Zhang@PSW.CN.WiFi.P2P.984851, 2017/05/05,
		//Add for add freq parm in p2p_find to p2p_search
		if (p2p->find_freq > 0 && (!(p2p->find_freq == 2412 || p2p->find_freq == 2437 || p2p->find_freq == 2462))) {
				type = P2P_SCAN_SOCIAL_PLUS_ONE;
				freq = p2p->find_freq;
		}
		//#endif /* VENDOR_EDIT */
...
void p2p_notify_scan_trigger_status(struct p2p_data *p2p, int status)
{
	if (status != 0) {
		...
		#ifndef VENDOR_EDIT
		//Laixin@PSW.CN.WiFi.Basic.P2P.1408501 2018/06/13
		//Modify for:when p2p scan was blocked, delay 2s to enter listen state to avoid driver busy.1194041
		p2p_continue_find(p2p);
		#else /* VENDOR_EDIT */
		p2p_dbg(p2p,"set a avoid-busy timeout before entering listen state.");
		eloop_cancel_timeout(confirm_p2p_state, p2p, NULL);
		eloop_register_timeout(P2P_SCAN_REQ_FAILED_AVOID_DRIVER_BUSY_TIMEOUT, 0, confirm_p2p_state,
						p2p, NULL);
		#endif /* VENDOR_EDIT */
	} else {
...
static void confirm_p2p_state(void *eloop_ctx, void *timeout_ctx)
{
  struct p2p_data *p2p = eloop_ctx;
	if(p2p->state == P2P_IDLE){
		wpa_printf(MSG_ERROR,"p2p state is idle scan stop");
		return ;
	}else{
		p2p_continue_find(p2p);
	}
	     ...
	     u8 seek_count, const char **seek, int freq)
{
	...
	//#ifdef VENDOR_EDIT
    //Longbo.Zhang@PSW.CN.WiFi.P2P.984851, 2017/05/05,
    //Add for record freq parm in p2p_find
    if (freq > 0)
        p2p->find_freq = freq;
    else
        p2p->find_freq = 0;
...
void p2p_go_complete(struct p2p_data *p2p, struct p2p_device *peer)
{
	...
	//solution:check group common freqlist,if common freq have 5g,but go frq set 2.4g
	//we dont save the persist group,after nexct connection come will do go negotiation again;
	os_memset(p2p->peer_addr, 0, ETH_ALEN);
	os_memcpy(p2p->peer_addr, peer->info.p2p_device_addr, ETH_ALEN);
	p2p_dbg(p2p, "P2P: p2p_go_complete--- ""peer Addr " MACSTR ")", MAC2STR(p2p->peer_addr));
	#endif /* VENDOR_EDIT */
