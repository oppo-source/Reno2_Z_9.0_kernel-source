...
static struct wpabuf * wps_build_m8(struct wps_data *wps)
{
    ...
    #ifdef VENDOR_EDIT
    //Longbo.Zhang@Connectivity.WiFi.Basic.P2P.1016183, 2017/05/28,
    //Add for pbc session might be exit if don't get WSC_DONE, remove it once send M8
    if (wps->pbc) {
        wpa_printf(MSG_DEBUG,"remove pbc session");
