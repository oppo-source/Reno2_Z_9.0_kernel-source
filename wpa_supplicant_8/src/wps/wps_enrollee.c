						...
						const struct wpabuf *msg)
{
	...
	case WPS_M2D:
		...
		u16 config_error = WPA_GET_BE16(attr.config_error);
		wpa_printf(MSG_DEBUG, "WPS: WPS_M2D %d", config_error);
		if (WPS_CFG_MULTIPLE_PBC_DETECTED == config_error){
		    wpa_printf(MSG_DEBUG, "WPS: WPS_M2D overlap detected by ap ");
