...
#ifndef VENDOR_EDIT
//Laixin@PSW.CN.WiFi.Basic.SoftAp.1190360, 2018/08/20
//Modify for: framework will handle the init acl, no need to set init acl in hostapd
static void hostapd_set_acl(struct hostapd_data *hapd)
#else /* VENDOR_EDIT */
static void __attribute__((unused)) hostapd_set_acl(struct hostapd_data *hapd)
#endif /* VENDOR_EDIT */
{
...
static int setup_interface2(struct hostapd_iface *iface)
{
	...
	if (hostapd_get_hw_features(iface)) {
		...
		if (ret == 2) {
			...
			if (err < 0) {
				int avai_chnls[] = {1, 6, 11};
				size_t avai_chnls_length = sizeof(avai_chnls) / sizeof(avai_chnls[0]);

				srand(time(NULL));
				iface->conf->channel = avai_chnls[rand() % avai_chnls_length];
				wpa_printf(MSG_INFO, "MTK ACS picked channel %d",
					iface->conf->channel);
			} else {
				wpa_printf(MSG_DEBUG,
						 ...
						 int err)
{
	...
	#ifndef VENDOR_EDIT
	//Laixin@PSW.CN.WiFi.Basic.SoftAp.1190360, 2018/08/20
	//Remove for: framework will handle the init acl, no need to set init acl in hostapd
