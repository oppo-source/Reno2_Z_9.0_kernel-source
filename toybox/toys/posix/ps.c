...
//#ifdef VENDOR_EDIT
//hanshiguang@PSW2, 2018/01/04, add for elsa
enum ELSA_STATUS {
    ELSA_START,
    ELSA_FOREGROUND,
    ELSA_BACKGROUND,
...
static int isFreeze3(int pid) {
    char fdstr[32];
    fdstr[0] = 0;
    sprintf(fdstr, "/data/system/elsa/freeze3/%d", pid);
    int ret = access(fdstr, F_OK);
    if (ret == 0) {
        return 1;
    } else {
        return 0;
...
#define LINE_MAX 100
//get the corresponding pid cgroups vetor
static int parseSplitProcCgroup(int pid, int* status) {
    char path[PATH_MAX];
    ...
    int cpusetStatus = 0;
    int freezeStatus = 0;
    int isFreezing3 = isFreeze3(pid);

    while(fgets(buffer, LINE_MAX, file) != NULL) {
        // Strip trailing \n that some sys files have.
        for ( int n = strlen(buffer); n > 0 && buffer[n] == '\n'; n-- )
        {
            buffer[n] = 0;
            break;
        }
        ...
        for (unsigned long i = 0; i < strlen(buffer); i++) {
            ...
            if (colonNum == 1) {
                cgroupType[j++] = buffer[i];
            }

            if (biasNum == 1) {
                cgroupName[k++] = buffer[i];
            }
        }
        ...
        if (strcmp(cgroupType, "cpuset") == 0) {
            if(!strcmp(cgroupName, "15") || !strcmp(cgroupName, "elsa_bg") || !strcmp(cgroupName, "background") || !strcmp(cgroupName, "system-background"))
                cpusetStatus |= BG;
            else if (!strcmp(cgroupName, "1") || !strcmp(cgroupName, "elsa_freezing1"))
                cpusetStatus |= F1;
            else if(!strcmp(cgroupName, "255") || !strcmp(cgroupName, "foreground"))
                cpusetStatus |= FG;
            else
                cpusetStatus |= FG;
        } else if (strcmp(cgroupType, "freezer") == 0) {
            if (strcmp(cgroupName, "elsa_freezing2") == 0) {
                freezeStatus |= F2;
            }
        }
    ...
    //printf ("cgroupStatus = %d, cpusetStatus = %d, freezeStatus = %d\n", cgroupStatus, cpusetStatus, freezeStatus);

    if (cpusetStatus & FG) {
        *status |= FG;
    }
    else if (cpusetStatus & BG) {
        *status |= BG;
    }
    else if (cpusetStatus & F1) {
        *status |= F1;
    }
...
static void show_ps_frz(struct procpid *tb) {
    int pid = *(tb->slot);
    if (pid < 0) return;

    int status = 0;
    parseSplitProcCgroup(pid, &status);
    char statusStr[1024];
    statusStr[0] = 0;

    if (status & FG)
        strcat(statusStr, "efg");
    if (status & BG)
        strcat(statusStr, "ebg");
    if (status & F1)
        strcat(statusStr, "fz1");
    if (status & F2)
        strcat(statusStr, "|fz2");
    if (status & F3)
        strcat(statusStr, "|fz3");
    if (status == 0) {
        strcpy(statusStr, "unk");
    }
...
void ps_main(void)
{
  ...
  if (!(toys.optflags&FLAG_M)) {
      char pre[256];
      char post[4096];
      char toybuf_with_frz[4096];
      pre[0] = 0;
      post[0] = 0;
      toybuf_with_frz[0] = 0;
      char* linename = "ADDR";
      char* post_with_ppid = strstr(toybuf, linename);
      if (post_with_ppid != NULL) {
          int pos = strlen(toybuf) - strlen(post_with_ppid);
          strncpy(pre, toybuf, pos + strlen(linename));
          pre[pos + strlen(linename)] = 0;
          int j = 0;
          for (int i = pos + strlen(linename); i < strlen(toybuf); i++) {
            post[j++] = toybuf[i];
          }
          post[j] = 0;
          sprintf(toybuf_with_frz, "%s\t%.10s %s", pre, "FRZ", post);
          printf("%.*s\n", TT.width, toybuf_with_frz);
      } else {
          printf("%.*s\n", TT.width, toybuf);
      }
  }
